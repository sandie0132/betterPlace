...
const EmpListBgvSummaryDownload = (props) => {
  useEffect(() => {
      ...
      </div>
      {
                props.data.data.status === 'inProgress' ? (
                  <div className={styles.maxWidth}>
                    <div className={cx(styles.headingLarge)}>bgv summary report generation is in progress</div>
                    <div className={cx(styles.textProgress, 'mt-2')}>
                      bgv summary report generation for
                      {' '}
                  ...
                  : props.data.data.status === 'success' ? (
                    <>
                      <div className={styles.maxWidth}>
                        <div className={cx(styles.headingLarge)}>bgv summary report generation has been successfull</div>
                        <div className={cx(styles.textSuccess, 'mt-2')}>
                          bgv summary report generation has been completed for
                          {' '}
                          {props.progressData.total}
                          {' '}
                      ...
                      <div className="ml-auto mt-5">
                        <DownloadButton
                          ...
                          label="download"
                          clickHandler={props.downloadBgvReportFileState === 'LOADING' ? null : () => props.onDownloadBgvReport(props.orgId, props.data._id, props.data.data.downloadURL, 'verify_summary_report')}
                    ...
                    : (
                      <>
                        <div className={styles.maxWidth}>
                          <div className={cx(styles.headingLarge)}>Bgv Summary Report generation failed !</div>
...
export default connect(mapStateToProps, mapDispatchToProps)(EmpListBgvSummaryDownload);
