...
import { initData, requiredFields } from './DrivingLicenseInitData';
...
import { validation, message } from './DrivingLicenseValidations';
///Icons
...
class DrivingLicense extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        //     this.props.onChange(this.state.formData, "DL")
        // }

        if (!_.isEqual(prevState.errors, this.state.errors)) {
            this.props.onError("DL", this.state.errors)
        }
        ...
        if (this.props.uploadFileState !== prevProps.uploadFileState && this.props.uploadFileState === 'SUCCESS') {
            ...
            const dlFile = this.props.dlFile;
            ...
            if (updatedFormData.uuid === undefined) {
                updatedFormData.downloadURL = [...updatedFormData.downloadURL, ...dlFile]
            }
            else {
                updatedFormData.downloadURL = [...updatedFormData.downloadURL, ...dlFile]
                // enableSubmit = true;
            ...
            this.setState({
                ...
                isEdited: true
            },()=> this.debouncedDataToParent(this.state.formData, 'DL'));
            this.props.onResetDownloadUrl();
        }
        if (this.props.deleteFileState !== prevProps.deleteFileState && this.props.deleteFileState === 'SUCCESS' && this.props.docType==='dlData') {
            let updatedFormData = _.cloneDeep(this.state.formData);
    ...
    handleInputChange = (value, inputIdentifier) => {
        ...
        this.setState({ formData: updatedFormData, isEdited: true }, ()=> this.debouncedDataToParent(this.state.formData, "DL") );
    }
    ...
    uploadFile = (file, dataId) => {
        ...
        this.props.onUploadFile('employee_documents', formData, 'dlData');

    }

    handleFileDownload = (url) => {
        this.props.onDownloadFile('employee_documents', url, 'dlData');
    }
    ...
    deleteFile = () => {
        ...
        this.props.onDeleteFile(dataId, 'employee_documents', fileName, url, 'dlData');
...
const mapStateToProps = state => {
    return {
        ...
        uploadFileState: state.empMgmt.empOnboard.additionalDetails.empDocuments.uploadDlState,
        ...
        dlFile: state.empMgmt.empOnboard.additionalDetails.empDocuments.dlFile,
