...
import styles from './Aadhaar.module.scss';
import { connect } from 'react-redux';
import cx from 'classnames';
import { initData, requiredFields } from './AadhaarInitData';
...
import { validation, message } from './AadhaarValidations';
...
class Aadhaar extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        //     this.props.onChange(this.state.formData, "AADHAAR")
        // }

        if (!_.isEqual(prevState.errors, this.state.errors)) {
            this.props.onError("AADHAAR", this.state.errors)
        }
        ...
        if (this.props.uploadFileState !== prevProps.uploadFileState && this.props.uploadFileState === 'SUCCESS') {
            ...
            const aadhaarFile = this.props.aadhaarFile;
            ...
            if (updatedFormData.uuid === undefined) {
                updatedFormData.downloadURL = [...updatedFormData.downloadURL, ...aadhaarFile]
            }
            else {
                updatedFormData.downloadURL = [...updatedFormData.downloadURL, ...aadhaarFile]
                // enableSubmit = true;
            ...
            this.setState({
                ...
                isEdited: true
            }, ()=> this.debouncedDataToParent(this.state.formData, 'AADHAAR'));
            this.props.onResetDownloadUrl();
        }
        if (this.props.deleteFileState !== prevProps.deleteFileState && this.props.deleteFileState === 'SUCCESS' && this.props.docType==='aadharData') {
            let updatedFormData = _.cloneDeep(this.state.formData);
    ...
    handleInputChange = (value, inputIdentifier) => {
        ...
        this.setState({ formData: updatedFormData, isEdited: true }, ()=> this.debouncedDataToParent(this.state.formData, "AADHAAR"));
    }
    ...
    uploadFile = (file, dataId) => {
        ...
        this.props.onUploadFile('employee_documents', formData, 'aadharData');

    }

    handleFileDownload = (url) => {
        this.props.onDownloadFile('employee_documents', url, 'aadharData');
    }
    ...
    deleteFile = () => {
        ...
        this.props.onDeleteFile(dataId, 'employee_documents', fileName, url, 'aadharData');
                ...
                <div className={cx('d-flex flex-column', this.props.isDisabled ? styles.showOpacity : null)} >
                    <div className="col-12 px-0">
                        ...
                        <div className="row no-gutters">
                            ...
                            <Input
                                name='yob'
                                className='col-2 pr-3'
                                label={'yob'}
                                type='text'
                                placeholder={'eg. "1997"'}
                                disabled={this.props.isDisabled}
                                required={_.includes(requiredFields, 'yob')}
                                value={this.state.formData.yob}
                                onChange={(value) => this.handleInputChange(value, 'yob')}
                                onError={(error) => this.handleError(error, 'yob')}
                                validation={validation['yob']}
                                message={message['yob']}
                                errors={this.state.errors['yob']}
                            />
                        </div>
...
const mapStateToProps = state => {
    return {
        ...
        uploadFileState: state.empMgmt.empOnboard.additionalDetails.empDocuments.uploadAadhaarState,
        ...
        aadhaarFile: state.empMgmt.empOnboard.additionalDetails.empDocuments.aadharFile,
...
export default (connect(mapStateToProps, mapDispatchToProps)(Aadhaar));