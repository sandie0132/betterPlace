...
import styles from './VoterCard.module.scss';
import { connect } from 'react-redux';
import cx from 'classnames';
import { initData, requiredFields } from './VoterCardInitData';
...
import { validation, message } from './VoterCardValidations';
...
class VoterCard extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        //     this.props.onChange(this.state.formData, "VOTER")
        // }

        if (!_.isEqual(prevState.errors, this.state.errors)) {
            this.props.onError("VOTER", this.state.errors)
        }
        ...
        if (this.props.uploadFileState !== prevProps.uploadFileState && this.props.uploadFileState === 'SUCCESS') {
            ...
            const voterFile = this.props.voterFile;
            ...
            if (updatedFormData.uuid === undefined) {
                updatedFormData.downloadURL = [...updatedFormData.downloadURL, ...voterFile]
            }
            else {
                updatedFormData.downloadURL = [...updatedFormData.downloadURL, ...voterFile]
                // enableSubmit = true;
            ...
            this.setState({
                ...
                isEdited: true
            }, ()=> this.debouncedDataToParent(this.state.formData, 'VOTER'));
            this.props.onResetDownloadUrl();
        }
        if (this.props.deleteFileState !== prevProps.deleteFileState && this.props.deleteFileState === 'SUCCESS' && this.props.docType==='voterData') {
            let updatedFormData = _.cloneDeep(this.state.formData);
    ...
    handleInputChange = (value, inputIdentifier) => {
        ...
        this.setState({ formData: updatedFormData, isEdited: true }, ()=> this.debouncedDataToParent(this.state.formData, "VOTER"));
    }
    ...
    uploadFile = (file, dataId) => {
        ...
        this.props.onUploadFile('employee_documents', formData, 'voterData');

    }

    handleFileDownload = (url) => {
        this.props.onDownloadFile('employee_documents', url, 'voterData');
    }
    ...
    deleteFile = () => {
        ...
        this.props.onDeleteFile(dataId, 'employee_documents', fileName, url, 'voterData');
...
const mapStateToProps = state => {
    return {
        ...
        uploadFileState: state.empMgmt.empOnboard.additionalDetails.empDocuments.uploadVoterState,
        ...
        voterFile: state.empMgmt.empOnboard.additionalDetails.empDocuments.voterFile,
...
export default (connect(mapStateToProps, mapDispatchToProps)(VoterCard));