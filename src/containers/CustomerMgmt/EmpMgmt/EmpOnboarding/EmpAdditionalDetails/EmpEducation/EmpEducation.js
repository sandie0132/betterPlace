...
import { initData } from './Education/EducationInitData';
import * as actions from './Store/action';

import addButton from "../../../../../../assets/icons/addMore.svg";
import styles from "./EmpEducation.module.scss";

class EmpEducation extends Component {
            ...
            if (!_.isEmpty(this.props.bgvData[0])) {
                if (!_.isEmpty(this.props.bgvData[0].bgv)) {
                    if (!_.isEmpty(this.props.bgvData[0].bgv.career)) {
                        let BgvArray = this.props.bgvData[0].bgv.career.checks.filter(({ education }) => education);
                        if (!_.isEmpty(BgvArray)) {
                            BgvData = BgvArray.find(data => data.education.uuid === uuid)
                        }
                    }
                }
            }
    ...
    getInProgressStatus = (uuid) => {
        ...
        if (!_.isEmpty(this.props.bgvData)) {
            if (!_.isEmpty(this.props.bgvData[0])) {
                if (!_.isEmpty(this.props.bgvData[0].bgv)) {
                    if (!_.isEmpty(this.props.bgvData[0].bgv.career)) {
                        BgvArray = this.props.bgvData[0].bgv.career.checks.filter(({ education }) => education);
                        if (!_.isEmpty(BgvArray)) {
                            BgvData = BgvArray.find(data => data.education.uuid === uuid)
                        }
    ...
    getBGVMissingInfo = (uuid) => {
        ...
        if (!_.isEmpty(missingInfo)) {
            if (!_.isEmpty(missingInfo["EDUCATION"])) {
                if (_.isEmpty(this.props.data)) {
                    missingData = missingInfo["EDUCATION"][0]
                } else {
                    let missingDataUuid = missingInfo["EDUCATION"].find(data => data.uuid === uuid)
        ...
        return (
            <div className={styles.formLayout} >
                <div className={cx("mb-4", this.props.isActive ? styles.opacityHeadIn : styles.opacityHeadOut)}>
                    ...
                    <span className={styles.formHead}>education </span>
                </div>
                <div>
                    <span 
                        className={styles.addHeading}
                        onClick={this.handleAddNewData}
                        disabled={this.state.disableSectionAddDelete}
                    >
                        add education
                        <img src={addButton} alt="add" className="ml-2" />
                    </span>
                </div>
                {
                    this.state.formData.map((data, index) => {
                        return (
                            <Education
                                ...
                                options={this.props.staticData['EMP_MGMT_EDUCATION_TYPE']}
...
const mapStateToProps = state => {
    return {
        error: state.empMgmt.empOnboard.additionalDetails.empEducation.error,
        uploadFileState: state.empMgmt.empOnboard.additionalDetails.empEducation.uploadFileState,
        fileUploadIndex: state.empMgmt.empOnboard.additionalDetails.empEducation.fileUploadIndex,
        uploadedFileUrl: state.empMgmt.empOnboard.additionalDetails.empEducation.uploadedFileUrl,
        downloadFileState: state.empMgmt.empOnboard.additionalDetails.empEducation.downloadFileState,
        downloadURL: state.empMgmt.empOnboard.additionalDetails.empEducation.downloadURL
    };
};
...
export default (connect(mapStateToProps, mapDispatchToProps)(EmpEducation));