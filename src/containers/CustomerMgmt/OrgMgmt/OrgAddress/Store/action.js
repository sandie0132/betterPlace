...
export const getDataList = (orgId) => {
    return (dispatch) => {
        ...
        axios.get(CUST_MGMT + `/org/${orgId}/address`)
            .then(response => {
...
export const postData = (orgId, data) => {
    return (dispatch, getState) => {
        ...
        axios.post(CUST_MGMT + `/org/${orgId}/address`, data)
            .then(response => {
                if (response.status === 200 || response.status === 201) {
                    const newData = response.data;
                    let updatedDataList = [newData,
                        ...getState().orgMgmt.orgAddress.dataList,
                    ]
                    dispatch({
...
export const putData = (orgId, dataId, data) => {
    return (dispatch, getState) => {
        ...
        axios.put(CUST_MGMT + `/org/${orgId}/address/${dataId}`, data)
            .then(response => {
                if (response.status === 200 || response.status === 201) {
                    let updatedDataList = getState().orgMgmt.orgAddress.dataList;
...
export const deleteData = (orgId, dataId) => {
    return (dispatch, getState) => {
        ...
        axios.delete(CUST_MGMT + `/org/${orgId}/address/${dataId}`)
            .then(response => {
                if (response.status === 200 || response.status === 201) {
                    let updatedDataList = getState().orgMgmt.orgAddress.dataList;
